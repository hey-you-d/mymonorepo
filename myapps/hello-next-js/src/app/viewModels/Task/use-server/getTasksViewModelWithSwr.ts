...
import { TaskModel, swrFetcher } from '@/app/models/Task/use-server/TaskModel';
...
export const getTaskViewModelWithSwr = async () => {
  ...
  const getTasksDBRows = useCallback(async () => {
    try {
      const result: Task[] = await taskModel.getTasksDBRows(`${BASE_URL}/api/tasks/v1/bff`);
